...
namespace ConsoleTableExt
{
    public class ConsoleTableBuilder
    {
        ...
        public static ConsoleTableBuilder From(DataTable dt)
        {
            ...
            foreach (DataRow row in dt.Rows) builder.Rows.Add(new List<object>(row.ItemArray));
        ...
        {
            if (list == null || !list.Any()) throw new ArgumentException($"{nameof(list)} cannot be null or empty");
        ...
        {
            if (rows == null || !rows.Any()) throw new ArgumentException($"{nameof(rows)} cannot be null or empty");

            var builder = new ConsoleTableBuilder();

            foreach (var row in rows) builder.Rows.Add(new List<object>(row));
        ...
        public static ConsoleTableBuilder From(List<List<object>> rows)
        {
            ...
            foreach (var row in rows) builder.Rows.Add(row);
        ...
        internal List<int> GetCadidateColumnLengths()
        {
            ...
            if (numberOfColumns < Column.Count) numberOfColumns = Column.Count;
            ...
            if (!columnLengths.Any()) throw new Exception("Table has no columns");
