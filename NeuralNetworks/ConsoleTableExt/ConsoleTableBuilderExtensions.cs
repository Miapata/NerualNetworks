...
namespace ConsoleTableExt
{
    public static class ConsoleTableBuilderExtensions
    {
        ...
        public static ConsoleTableBuilder WithOptions(this ConsoleTableBuilder builder,
            ConsoleTableBuilderOption options)
        {
        ...
        {
            if (!builder.Rows.Any()) throw new Exception("Table has no rows");

            var numberOfColumns = builder.Rows.Max(x => x.Count);

            if (numberOfColumns < builder.Column.Count) numberOfColumns = builder.Column.Count;

            for (var i = 0; i < 1; i++)
                if (builder.Column.Count < numberOfColumns)
                {
                    var missCount = numberOfColumns - builder.Column.Count;
                    for (var j = 0; j < missCount; j++) builder.Column.Add(null);
                }
            ...
            for (var i = 0; i < builder.Rows.Count; i++)
                if (builder.Rows[i].Count < numberOfColumns)
                {
                    ...
                    for (var j = 0; j < missCount; j++) builder.Rows[i].Add(null);
                }
        ...
        private static StringBuilder CreateTableForDefaultFormat(ConsoleTableBuilder builder)
        {
            ...
            if (format == string.Empty) return strBuilder;

            // find the longest formatted line
            var maxRowLength = Math.Max(0,
                builder.Rows.Any() ? builder.Rows.Max(row => string.Format(format, row.ToArray()).Length) : 0);
            ...
            // header
            if (builder.Column != null && builder.Column.Any() &&
                builder.Column.Max(x => (x ?? string.Empty).ToString().Length) > 0)
            {
        ...
        private static StringBuilder CreateTableForMarkdownFormat(ConsoleTableBuilder builder)
        {
            ...
            // add each row
            var results = builder.Rows.Skip(skipFirstRow ? 1 : 0).Select(row => string.Format(format, row.ToArray()))
                .ToList();
        ...
        {
            var result = new StringBuilder().AppendFormat(builder.Options.MetaRowFormat, builder.Options.MetaRowParams)
                .ToString();
            ...
            if (result.Contains(AppConstants.MetaRow.COLUMN_COUNT))
                result = result.Replace(AppConstants.MetaRow.COLUMN_COUNT,
                    builder.GetCadidateColumnLengths().Count.ToString());
